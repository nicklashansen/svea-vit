...
class RandomizationWrapper(gym.Wrapper):
	"""Wrapper for the randomization experiments"""
	...
	def reset(self):
		...
		elif 'dynamics' in self._mode:
			if 'easy' in self._mode:
				mass = self._random_state.uniform(0.5, 1.75, size=1)
				damping = self._random_state.uniform(0.75, 1.5, size=1)
			elif 'hard' in self._mode:
				mass = self._random_state.uniform(0.25, 2.0, size=1)
				damping = self._random_state.uniform(0.5, 1.5, size=1)
			friction = self._random_state.uniform(0.9, 1.1, size=1)
			setting_kwargs = {
				'mass': mass,
				'damping': damping,
				'friction': friction
